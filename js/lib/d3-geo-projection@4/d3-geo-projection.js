//import {geoProjectionMutator, geoProjection, geoAzimuthalEquidistantRaw, geoAzimuthalEqualAreaRaw, geoCentroid, geoRotation, geoEquirectangular, geoOrthographic, geoCircle, geoStream, geoBounds, geoInterpolate, geoGnomonic, geoGnomonicRaw} from "/-/d3-geo@v3.0.1-kwyelOm8gApBxT2oVVB9/dist=es2019,mode=imports/optimized/d3-geo.js";
//import {range, merge} from "/-/d3-array@v3.1.1-Ibshj34oOmCw8da1RLSW/dist=es2019,mode=imports/optimized/d3-array.js";

// local
import {geoProjectionMutator, geoProjection, geoAzimuthalEquidistantRaw, geoAzimuthalEqualAreaRaw, geoCentroid, geoRotation, geoEquirectangular, geoOrthographic, geoCircle, geoStream, geoBounds, geoInterpolate, geoGnomonic, geoGnomonicRaw} from "./d3-geo.js";
import {range, merge} from "./d3-array.js";
...
function airy() {
  var beta = halfPi, m = geoProjectionMutator(airyRaw), p = m(beta);
  p.radius = function(_) {
...
function aitoff() {
  return geoProjection(aitoffRaw).scale(152.63);
}
...
function armadillo() {
  var phi02 = 20 * radians, sPhi0 = phi02 >= 0 ? 1 : -1, tanPhi0 = tan(sPhi0 * phi02), m = geoProjectionMutator(armadilloRaw), p = m(phi02), stream_ = p.stream;
...
function august() {
  return geoProjection(augustRaw).scale(66.1603);
}
...
function baker() {
  return geoProjection(bakerRaw).scale(112.314);
}
...
function berghaus() {
  var lobes2 = 5, m = geoProjectionMutator(berghausRaw), p = m(lobes2), projectionStream = p.stream, epsilon3 = 0.01, cr = -cos(epsilon3 * radians), sr = sin(epsilon3 * radians);
  p.lobes = function(_) {
  ...
  p.stream = function(stream) {
    ...
    rotateStream.sphere = function() {
      ...
      for (var i = 0, delta = 360 / lobes2, delta0 = 2 * pi / lobes2, phi = 90 - 180 / lobes2, phi02 = halfPi; i < lobes2; ++i, phi -= delta, phi02 -= delta0) {
        ...
        if (phi < -90) {
          sphereStream.point(-90, -180 - phi - epsilon3);
          sphereStream.point(-90, -180 - phi + epsilon3);
        } else {
          sphereStream.point(90, phi + epsilon3);
          sphereStream.point(90, phi - epsilon3);
        }
      }
      sphereStream.lineEnd(), sphereStream.polygonEnd();
...
function hammerRaw(A2, B2) {
  if (arguments.length < 2)
    B2 = A2;
  if (B2 === 1)
    return geoAzimuthalEqualAreaRaw;
  if (B2 === Infinity)
    return hammerQuarticAuthalicRaw;
  function forward(lambda, phi) {
    var coordinates = geoAzimuthalEqualAreaRaw(lambda / B2, phi);
    coordinates[0] *= A2;
    return coordinates;
  }
  forward.invert = function(x, y) {
    var coordinates = geoAzimuthalEqualAreaRaw.invert(x / A2, y);
    coordinates[0] *= B2;
    return coordinates;
  };
  return forward;
...
function hammer() {
  var B2 = 2, m = geoProjectionMutator(hammerRaw), p = m(B2);
...
function bertin1953Raw() {
  var hammer2 = hammerRaw(1.68, 2), fu = 1.4, k2 = 12;
  function forward(lambda, phi) {
    if (lambda + phi < -fu) {
...
function bertin() {
  return geoProjection(bertin1953Raw()).rotate([-16.5, -42]).scale(176.57).center([7.93, 0.09]);
}
...
function mollweide() {
  return geoProjection(mollweideRaw).scale(169.529);
}
...
function boggs() {
  return geoProjection(boggsRaw).scale(160.857);
}
function parallel1(projectAt) {
  var phi02 = 0, m = geoProjectionMutator(projectAt), p = m(phi02);
  p.parallel = function(_) {
...
function sinusoidal() {
  return geoProjection(sinusoidalRaw).scale(152.63);
}
...
function bottomley() {
  var sinPsi = 0.5, m = geoProjectionMutator(bottomleyRaw), p = m(sinPsi);
...
function bromley() {
  return geoProjection(bromleyRaw).scale(152.63);
}
...
function chamberlin(p0, p1, p2) {
  var c = geoCentroid({type: "MultiPoint", coordinates: [p0, p1, p2]}), R = [-c[0], -c[1]], r = geoRotation(R), f = chamberlinRaw(pointRadians(r(p0)), pointRadians(r(p1)), pointRadians(r(p2)));
  f.invert = solve2d(f);
  var p = geoProjection(f).rotate(R), center = p.center;
  delete p.rotate;
  p.center = function(_) {
...
function collignon() {
  return geoProjection(collignonRaw).scale(95.6464).center([0, 30]);
}
...
function craster() {
  return geoProjection(crasterRaw).scale(156.19);
}
...
function eckert1() {
  return geoProjection(eckert1Raw).scale(165.664);
}
...
function eckert2() {
  return geoProjection(eckert2Raw).scale(165.664);
}
...
function eckert3() {
  return geoProjection(eckert3Raw).scale(180.739);
}
...
function eckert4() {
  return geoProjection(eckert4Raw).scale(180.739);
}
...
function eckert5() {
  return geoProjection(eckert5Raw).scale(173.044);
}
...
function eckert6() {
  return geoProjection(eckert6Raw).scale(173.044);
}
...
function eisenlohr() {
  return geoProjection(eisenlohrRaw).scale(62.5271);
}
...
function fahey() {
  return geoProjection(faheyRaw).scale(137.152);
}
...
function foucaut() {
  return geoProjection(foucautRaw).scale(135.264);
}
...
function foucautSinusoidal() {
  var alpha = 0.5, m = geoProjectionMutator(foucautSinusoidalRaw), p = m(alpha);
  p.alpha = function(_) {
...
function gingeryRaw(rho, n) {
  ...
  function forward(lambda, phi) {
    var p = geoAzimuthalEquidistantRaw(lambda, phi), x = p[0], y = p[1], r2 = x * x + y * y;
    if (r2 > rho2) {
...
function gingery() {
  var n = 6, rho = 30 * radians, cRho = cos(rho), sRho = sin(rho), m = geoProjectionMutator(gingeryRaw), p = m(rho, n), stream_ = p.stream, epsilon3 = 0.01, cr = -cos(epsilon3 * radians), sr = sin(epsilon3 * radians);
  p.radius = function(_) {
    if (!arguments.length)
      return rho * degrees;
...
function ginzburg8() {
  return geoProjection(ginzburg8Raw).scale(131.747);
}
var ginzburg9Raw = ginzburgPolyconicRaw(2.6516, -0.76534, 0.19123, -0.047094, 1.36289, -0.13965, 0.031762);
function ginzburg9() {
  return geoProjection(ginzburg9Raw).scale(131.087);
}
...
function gringorten() {
  return geoProjection(squareRaw(gringortenRaw)).scale(239.75);
}
...
function guyou() {
  return geoProjection(squareRaw(guyouRaw)).scale(151.496);
}
...
function hammerRetroazimuthal() {
  var phi02 = 0, m = geoProjectionMutator(hammerRetroazimuthalRaw), p = m(phi02), rotate_ = p.rotate, stream_ = p.stream, circle = geoCircle();
  p.parallel = function(_) {
    if (!arguments.length)
      return phi02 * degrees;
    var r = p.rotate();
  ...
  p.stream = function(stream) {
    ...
    stream.sphere = function() {
      stream.polygonStart();
      var epsilon3 = 0.01, ring = circle.radius(90 - epsilon3)().coordinates[0], n = ring.length - 1, i = -1, p2;
      stream.lineStart();
      while (++i < n)
        stream.point((p2 = ring[i])[0], p2[1]);
      stream.lineEnd();
      ring = circle.radius(90 + epsilon3)().coordinates[0];
      n = ring.length - 1;
      stream.lineStart();
      while (--i >= 0)
        stream.point((p2 = ring[i])[0], p2[1]);
...
function healpix() {
  var H = 4, m = geoProjectionMutator(healpixRaw), p = m(H), stream_ = p.stream;
  p.lobes = function(_) {
...
function hill() {
  var K2 = 1, m = geoProjectionMutator(hillRaw), p = m(K2);
  p.ratio = function(_) {
...
function sinuMollweide() {
  return geoProjection(sinuMollweideRaw).rotate([-20, -55]).scale(164.263).center([0, -5.4036]);
}
...
function homolosine() {
  return geoProjection(homolosineRaw).scale(152.63);
}
function hufnagelRaw(a, b, psiMax, ratio) {
  var k2 = sqrt(4 * pi / (2 * psiMax + (1 + a - b / 2) * sin(2 * psiMax) + (a + b) / 2 * sin(4 * psiMax) + b / 2 * sin(6 * psiMax))), c = sqrt(ratio * sin(psiMax) * sqrt((1 + a * cos(2 * psiMax) + b * cos(4 * psiMax)) / (1 + a + b))), M = psiMax * mapping(1);
  function radius(psi) {
...
function hufnagel() {
  var a = 1, b = 0, psiMax = 45 * radians, ratio = 2, mutate = geoProjectionMutator(hufnagelRaw), projection = mutate(a, b, psiMax, ratio);
...
function hyperelliptical() {
  var alpha = 0, k2 = 2.5, gamma = 1.183136, m = geoProjectionMutator(hyperellipticalRaw), p = m(alpha, k2, gamma);
  p.alpha = function(_) {
...
function interrupt(project, lobes2, inverse2) {
  var sphere2, bounds;
  function forward(lambda, phi) {
    var sign2 = phi < 0 ? -1 : 1, lobe = lobes2[+(phi < 0)];
  ...
  var p = geoProjection(forward), stream_ = p.stream;
  p.stream = function(stream) {
    var rotate = p.rotate(), rotateStream = stream_(stream), sphereStream = (p.rotate([0, 0]), stream_(stream));
    p.rotate(rotate);
    rotateStream.sphere = function() {
      geoStream(sphere2, sphereStream);
    };
    return rotateStream;
  };
  p.lobes = function(_) {
    if (!arguments.length)
      return lobes2.map(function(lobe) {
...
function kavrayskiy7() {
  return geoProjection(kavrayskiy7Raw).scale(158.837);
}
...
function lagrange() {
  var n = 0.5, m = geoProjectionMutator(lagrangeRaw), p = m(n);
...
function larrivee() {
  return geoProjection(larriveeRaw).scale(97.2672);
}
...
function laskowski() {
  return geoProjection(laskowskiRaw).scale(139.98);
}
...
function littrow() {
  return geoProjection(littrowRaw).scale(144.049).clipAngle(90 - 1e-3);
}
...
function miller() {
  return geoProjection(millerRaw).scale(108.318);
}
...
function modifiedStereographic(coefficients, rotate) {
  var p = geoProjection(modifiedStereographicRaw(coefficients)).rotate(rotate).clipAngle(90), r = geoRotation(rotate), center = p.center;
  delete p.rotate;
  p.center = function(_) {
    return arguments.length ? center(r(_)) : r.invert(center());
  };
...
function mtFlatPolarParabolic() {
  return geoProjection(mtFlatPolarParabolicRaw).scale(164.859);
}
...
function mtFlatPolarQuartic() {
  return geoProjection(mtFlatPolarQuarticRaw).scale(188.209);
}
...
function mtFlatPolarSinusoidal() {
  return geoProjection(mtFlatPolarSinusoidalRaw).scale(166.518);
}
...
function naturalEarth2() {
  return geoProjection(naturalEarth2Raw).scale(175.295);
}
...
function nellHammer() {
  return geoProjection(nellHammerRaw).scale(152.63);
}
var lobes$6 = [[
  [[-180, 0], [-90, 90], [0, 0]],
  [[0, 0], [90, 90], [180, 0]]
...
function nicolosi() {
  return geoProjection(nicolosiRaw).scale(127.267);
}
...
function polyconic() {
  return geoProjection(polyconicRaw).scale(103.74);
}
function matrix(a, b) {
...
function polyhedral(root, face) {
  ...
  var proj = geoProjection(forward), stream_ = proj.stream;
...
function outline(stream, node, parent) {
  var point, edges = node.edges, n = edges.length, edge, multiPoint = {type: "MultiPoint", coordinates: node.face}, notPoles = node.face.filter(function(d) {
    return abs(d[1]) !== 90;
  }), b = geoBounds({type: "MultiPoint", coordinates: notPoles}), inside = false, j = -1, dx = b[1][0] - b[0][0];
  var c = dx === 180 || dx === 360 ? [(b[0][0] + b[1][0]) / 2, (b[0][1] + b[1][1]) / 2] : geoCentroid(multiPoint);
  if (parent)
    while (++j < n) {
      if (edges[j] === parent)
        break;
...
function waterman(faceProjection) {
  faceProjection = faceProjection || function(face2) {
    var c = face2.length === 6 ? geoCentroid({type: "MultiPoint", coordinates: face2}) : face2[0];
    return geoGnomonic().scale(1).translate([0, 0]).rotate([-c[0], -c[1]]);
  };
...
function quincuncial(project) {
  ...
  return geoProjection(projectQuincuncial).rotate([-90, -90, 45]).clipAngle(180 - 1e-3);
}
...
robinsonRaw.invert = function(x, y) {
  ...
  do {
    var ay = K$1[i0][1], by = K$1[i0 + 1][1], cy = K$1[min(19, i0 + 2)][1], u = cy - ay, v = cy - 2 * by + ay, t = 2 * (abs(y) - by) / u, c = v / u, di = t * (1 - c * t * (1 - 2 * c * t));
    if (di >= 0 || i0 === 1) {
      phi = (y >= 0 ? 5 : -5) * (di + i);
      var j = 50, delta;
      do {
...
function satellite() {
  var distance2 = 2, omega = 0, m = geoProjectionMutator(satelliteRaw), p = m(distance2, omega);
...
function times() {
  return geoProjection(timesRaw).scale(146.153);
}
function twoPoint(raw, p0, p1) {
  var i = geoInterpolate(p0, p1), o = i(0.5), a = geoRotation([-o[0], -o[1]])(p0), b = i.distance / 2, y = -asin(sin(a[1] * radians) / sin(b)), R = [-o[0], -o[1], -(a[0] > 0 ? pi - y : y) * degrees], p = geoProjection(raw(b)).rotate(R), r = geoRotation(R), center = p.center;
  delete p.rotate;
  p.center = function(_) {
    return arguments.length ? center(r(_)) : r.invert(center());
  };
...
function twoPointEquidistantRaw(z0) {
  if (!(z0 *= 2))
    return geoAzimuthalEquidistantRaw;
...
function vanDerGrinten() {
  return geoProjection(vanDerGrintenRaw).scale(79.4183);
}
...
function vanDerGrinten2() {
  return geoProjection(vanDerGrinten2Raw).scale(79.4183);
}
...
function vanDerGrinten3() {
  return geoProjection(vanDerGrinten3Raw).scale(79.4183);
}
...
function vanDerGrinten4() {
  return geoProjection(vanDerGrinten4Raw).scale(127.16);
}
...
function wagner() {
  var poleline = 65 * radians, parallels = 60 * radians, inflation = 20, ratio = 200, mutate = geoProjectionMutator(wagnerRaw), projection = mutate(poleline, parallels, inflation, ratio);
...
function wagner4() {
  return geoProjection(wagner4Raw).scale(176.84);
}
...
function wagner6() {
  return geoProjection(wagner6Raw).scale(152.63);
}
...
function wiechel() {
  return geoProjection(wiechelRaw).rotate([0, -90, 45]).scale(124.75).clipAngle(180 - 1e-3);
}
...
function winkel3() {
  return geoProjection(winkel3Raw).scale(158.837);
}
